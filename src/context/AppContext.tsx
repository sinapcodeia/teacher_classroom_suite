 ...
 *
 * ESTRATEGIA DE PESO PROPORCIONAL:
 * Si un pilar no tiene ninguna nota registrada aún, su peso se redistribuye
 * entre los pilares que sí tienen datos, evitando sesgo hacia abajo en notas
 * parciales (ej: alumno nuevo que solo tiene SBH registrado).
 *
...
interface AppContextType {
  ...
  addGradesBatch: (entries: { studentId: string; grade: Omit<Grade, "id"> }[]) => Promise<string[]>;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // SAFETY TIMEOUT: Salida ultrarrápida si Auth tarda más de 1.2s
    const safetyTimeout = setTimeout(() => {
      setAuthLoading(prev => {
        if (prev) console.log("Carga ultrarrápida: resolviendo sesión inmediatamente.");
        return false;
      });
    }, 1200);
  ...
  // ── PERSISTENCIA LOCAL CON DEBOUNCE ────────────────────────────────────────
  // Escritura diferida 400ms para evitar micro-freezes en dispositivos lentos (USB).
  // Si masterData o subjects cambian varias veces en ráfaga, solo escribe una vez.
  useEffect(() => {
  ...
  const updateSingleDetailedGrade = async (studentId: string, subjectId: string, periodId: string, category: keyof DetailedGrades, index: number, score: number | null) => {
    try {
      ...
      // ── DEEP COPY SEGURO para no mutar el estado de React directamente ──
      const existingPeriodData = student.detailedGrades?.[subjectId]?.[periodId];
      const currentSubjectGrades: DetailedGrades = {
        sb:  existingPeriodData?.sb  ? [...existingPeriodData.sb]  : Array(8).fill(null),
        sbh: existingPeriodData?.sbh ? [...existingPeriodData.sbh] : Array(8).fill(null),
      ...
      if (category === 'aut') {
        ...
        // Asegurar que el array tiene suficiente longitud para el slot solicitado
        const arr = currentSubjectGrades[category] as (number | null)[];
        while (arr.length <= index) arr.push(null);
        arr[index] = score;
        currentSubjectGrades[category] = arr as any;
      }
  ...
  /**
   * Agrega o actualiza UNA nota en el historial de sesión de un estudiante.
   * Estrategia: optimistic update local inmediato + escritura a Firestore asíncrona.
   * Para guardar N notas de golpe (modo lista), usa addGradesBatch.
   */
  const addGrade = async (studentId: string, grade: Omit<Grade, "id">) => {
    try {
      ...
      // 1. Optimistic update local (instantáneo, sin esperar red)
      setStudents(prev => prev.map(s => s.id === studentId ? { ...s, ...updates } : s));

      // 2. Sincronizar a Firestore (no bloquea el flujo si falla)
      await updateDoc(doc(db, "students", studentId), updates);
    } catch (err) {
      console.error("Error al añadir o actualizar nota:", err);
      // No re-throw: el estado local ya es correcto; se reintentará en próximo onSnapshot
    }
  };

  /**
   * Guarda N notas en Firestore usando WriteBatch (máx 500 por lote).
   * Usar en modo lista cuando se califica a múltiples estudiantes a la vez.
   * Retorna un array de IDs de estudiantes actualizados correctamente.
   */
  const addGradesBatch = async (
    entries: { studentId: string; grade: Omit<Grade, "id"> }[]
  ): Promise<string[]> => {
    if (entries.length === 0) return [];
    const BATCH_SIZE = 490; // Firestore límite: 500 ops/batch, dejamos margen
    const successIds: string[] = [];

    // Preparar todas las actualizaciones en memoria primero
    const allUpdates: { studentId: string; updates: { grades: Grade[]; avgGrade: number } }[] = [];
    ...
    // Optimistic update masivo local (instantáneo)
    setStudents(prev => {
      const map = new Map(prev.map(s => [s.id, s]));
      for (const { studentId, updates } of allUpdates) {
        const existing = map.get(studentId);
        if (existing) map.set(studentId, { ...existing, ...updates });
      }
      return Array.from(map.values());
    });

    // Escrituras por lotes a Firestore
    try {
      for (let i = 0; i < allUpdates.length; i += BATCH_SIZE) {
        const lote = writeBatch(db);
        const porcion = allUpdates.slice(i, i + BATCH_SIZE);
        for (const { studentId, updates } of porcion) {
          lote.update(doc(db, "students", studentId), updates);
          successIds.push(studentId);
        }
        await lote.commit();
      }
    } catch (err) {
      console.error("Error en batch de notas:", err);
      // Los datos locales ya son correctos; Firestore se reconciliará en el próximo onSnapshot
    }
